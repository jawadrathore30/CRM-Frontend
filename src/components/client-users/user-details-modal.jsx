...
function UserDetailsModal({ user, onClose }) {
	...
	const getSocialUrl = (platform, handle) => {
		...
		switch (platform) {
			...
			case "whatsapp":
				// WhatsApp links typically use phone numbers
	...
	// Check if user has any social profiles (only telegram and whatsapp now)
	const hasSocialProfiles = user.telegram || user.whatsapp;

	// Check if user is company owner (example logic)
	const isCompanyOwner =
		user.role === "Admin" ||
		user.position?.toLowerCase().includes("owner") ||
					...
					<div className="px-8 py-6 flex flex-col md:flex-row md:items-center gap-6 bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700">
						...
						<div className="flex flex-col">
							<div className="flex flex-wrap items-end gap-2 mt-1">
								...
								{/* Replace role and status badges with company owner indicator */}
								<span
									className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium backdrop-blur-sm border ${
										isCompanyOwner
											? "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20"
											: "bg-slate-500/10 text-slate-600 dark:text-slate-400 border-slate-500/20"
									}`}>
									{isCompanyOwner ? "Owner" : "User"}
								</span>
							</div>

							<div className="flex items-center gap-2 mt-3">
								{/* Add WhatsApp button before Message and Edit buttons */}
								<Button
									variant="outline"
									size="sm"
									className="h-8 gap-1 text-green-600 dark:text-green-400 border-green-200 dark:border-green-700/50 bg-green-50/50 dark:bg-green-900/20 backdrop-blur-sm hover:bg-green-100/50 dark:hover:bg-green-900/30">
									...
									{!hasSocialProfiles ? (
										<div className="flex flex-col items-center justify-center py-12 text-center">
											...
											<h3 className="text-lg font-medium text-slate-700 dark:text-slate-300 mb-2">
												No Messaging Profiles
											</h3>
											<p className="text-sm text-slate-500 dark:text-slate-400 max-w-md">
												This user hasn't added any messaging contact information
												yet.
											</p>
										</div>
									) : (
